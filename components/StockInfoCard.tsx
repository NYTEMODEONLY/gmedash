  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 h-full">
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-600 rounded-full">NYSE</span>
          </div>
      ...
      <div className={`mb-6 p-4 rounded-lg transition-colors ${
        priceFlash === 'up' ? 'bg-green-50' :
        priceFlash === 'down' ? 'bg-red-50' : 'bg-gray-50'
      }`}>
      ...
      <div className="mb-6">
        ...
        <div className="relative h-2 bg-gray-200 rounded-full overflow-hidden">
          ...
          />
          <div
            className="absolute w-3 h-3 bg-gray-900 rounded-full shadow-md -top-0.5 transform -translate-x-1/2"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-gray-50 rounded-lg p-3">
          ...
          <div className="text-sm font-semibold text-gray-900">${stockData.open.toFixed(2)}</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-3">
          <div className="text-xs text-gray-500 mb-1">Prev Close</div>
          <div className="text-sm font-semibold text-gray-900">${stockData.previousClose.toFixed(2)}</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-3">
          <div className="text-xs text-gray-500 mb-1">Day High</div>
          <div className="text-sm font-semibold text-green-600">${stockData.high.toFixed(2)}</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-3">
          <div className="text-xs text-gray-500 mb-1">Day Low</div>
          <div className="text-sm font-semibold text-red-600">${stockData.low.toFixed(2)}</div>
        </div>
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-gray-100">
        <div className="flex flex-wrap gap-2">
          <a
            ...
            className="text-xs px-2.5 py-1 bg-purple-50 text-purple-700 rounded-full hover:bg-purple-100 transition-colors"
          >
            Yahoo Finance
          </a>
          <a
      ...
      {stockData.source && (
        <div className="mt-3 flex items-center justify-between text-xs">
          <span className={`px-2 py-0.5 rounded ${
            stockData.source === 'finnhub' ? 'bg-green-100 text-green-700' :
            stockData.source === 'yahoo' ? 'bg-purple-100 text-purple-700' :
