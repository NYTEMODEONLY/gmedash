...
### Key Directories
- **app/api/** - 12 API route handlers with caching layer
...
|--------|---------|-------|
| Yahoo Finance chart API | Stock quotes and historical data | 30s (market open) / 5m (closed) |
...
| Options flow | Unavailable until a reliable free public source exists | 5 min unavailable response |
...
- Do not re-add Ryan Cohen/X widgets or mirror scraping without proper API access and clear source attribution.
